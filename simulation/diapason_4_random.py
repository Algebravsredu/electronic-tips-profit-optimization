...
p = dp.probabilities_random(d[0], d[1], set.index[:5], set.index, 2700, 3300)
...
for i in range(6000):
    for j in range(12):
        if check_sum_type[i] == j:
            check_sum.append(random.randrange(2700+j * 50, 2700 + (j+1) * 50, 1))
...
st.to_csv('/Users/Egor/Desktop/все/Результаты симуляций/diapason_4_random')
