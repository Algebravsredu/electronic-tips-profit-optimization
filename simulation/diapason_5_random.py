...
p = dp.probabilities_random(d[0], d[1], set.index[:5], set.index, 3300, 4100)

 # #создание массива сумм чека с исходным распределением
check_sum = []
check_sum_type = np.random.choice(range(16), 9000, p = p[3])

for i in range(9000):
    for j in range(16):
        if check_sum_type[i] == j:
            check_sum.append(random.randrange(3300 + j * 50, 3300 + (j+1) * 50, 1))
...
st.to_csv('/Users/Egor/Desktop/все/Результаты симуляций/diapason_5_random')
