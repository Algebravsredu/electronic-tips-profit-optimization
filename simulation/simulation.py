...
def model_percent(check_sum, num_simulation, type_of_button_prob, type_of_percent_prob, buttons_set, ds):
    ...
    for i in range(num_simulation):
        ...
        for j in range(len(check_sum)):
            if select_type_of_button[j] == 0:
                tips_amount.append(np.random.choice(ds[ds['type_of_selected_button'] == 0]['amount']))
