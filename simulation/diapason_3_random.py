...
p = dp.probabilities_random(d[0], d[1], set.index[:5], set.index, 2100, 2700)

 # #создание массива сумм чека с исходным распределением
check_sum = []
check_sum_type = np.random.choice(range(12), 6000, p = p[3])
for i in range(6000):
    for j in range(12):
        if check_sum_type[i] == j:
            check_sum.append(random.randrange(2100+j * 50, 2100 + (j+1) * 50, 1))
...
st.to_csv('/Users/Egor/Desktop/все/Результаты симуляций/diapason_3_random')
